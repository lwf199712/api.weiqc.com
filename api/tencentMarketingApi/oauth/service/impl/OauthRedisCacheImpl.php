...
class OauthRedisCacheImpl extends BaseObject implements OauthCacheService
{
    ...
    {
        $this->redisUtils->getRedis()->hdel(Yii::$app->params['oauth']['tencent_marketing_api']['token_key'], [$oauthDto->authorizer_info->account_uin]);
        if (!$this->redisUtils->getRedis()->set(Yii::$app->params['oauth']['tencent_marketing_api']['token_key'] . $oauthDto->authorizer_info->account_uin,
        ...
        $this->redisUtils->getRedis()->expire(Yii::$app->params['oauth']['tencent_marketing_api']['token_key'] . $oauthDto->authorizer_info->account_uin,
            $oauthDto->refresh_token_expires_in - time());
    }
    ...
    {
        //从redis中获取token
        $oauthRedisDto = $this->redisUtils->getRedis()->get(Yii::$app->params['oauth']['tencent_marketing_api']['token_key'] . $accountUin);
        if ($oauthRedisDto) {
